    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="relative space-y-1" ref={dropdownRef}>
          <label className="text-foreground block text-sm font-medium">Pilih Kategori Game</label>
          <input type="hidden" name="game_id" value={selectedGameId} required />
          <button
            type="button"
            onClick={() => setIsOpen(!isOpen)}
            className="border-border bg-muted text-foreground hover:bg-muted flex w-full items-center justify-between rounded-[10px] border px-3 py-2 transition-colors focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none"
          >
            ...
            <div className="border-border bg-card absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-[10px] border py-1">
              {games.map((game) => (
                <button
                  ...
                  className="text-foreground hover:bg-muted flex w-full items-center justify-between px-3 py-2 text-left text-sm transition-colors"
                >
                  {game.name}
      ...
      <div className="space-y-2">
            ...
            {images.map((img, idx) => (
              ...
              >
                ...
                <button
                  ...
                  className="bg-card/90 text-foreground hover:bg-card absolute top-1 right-1 rounded-[10px] p-1 opacity-0 transition-opacity group-hover:opacity-100 hover:text-rose-600"
                >
                  <X className="h-3 w-3" />
                </button>
              </div>
        ...
        {images.length < 20 && (
          <div className="group border-border bg-muted hover:bg-muted relative flex h-32 w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded-[10px] border-2 border-dashed transition-colors">
            <div className="text-muted-foreground flex flex-col items-center justify-center pt-5 pb-6">
              <UploadCloud className="text-faint-foreground mb-3 h-8 w-8 transition-colors group-hover:text-blue-500" />
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="space-y-1">
          <label className="text-foreground block text-sm font-medium" htmlFor="capital_price">
            Harga Modal (Rp)
          </label>
          <input
            id="capital_price"
