...
export function ExportButton({ status, gameId }: { status?: string; gameId?: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-2 rounded-[10px] border border-border bg-card px-4 py-2.5 text-sm font-medium text-foreground transition-all hover:bg-muted disabled:opacity-50"
    >
      {isExporting ? (
