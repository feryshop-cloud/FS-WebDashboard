...
const BUY_STATUS_OPTIONS = ["pending", "processing", "success", "failed"];
...
const BUY_STATUS_LABEL: Record<string, string> = {
  pending: "Menunggu",
  processing: "Diproses",
  success: "Sukses",
  failed: "Gagal",
...
function getBuyBadgeClass(status: string) {
  ...
  if (status === "processing") return "bg-orange-50 text-orange-600 border-orange-100";
  if (status === "failed") return "bg-rose-50 text-rose-600 border-rose-100";
                ...
                orders.map((order) => (
                  <tr key={order.id} className="group transition-colors hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      ...
                      >
                        {BUY_STATUS_LABEL[order.buy_status] || order.buy_status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center text-sm font-medium whitespace-nowrap">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-md border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-xs font-semibold text-blue-600 transition-colors hover:bg-blue-100"
                      >
