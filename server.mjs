...
const IGNORE_RE =
  /^\/(_next\/static|_next\/image|favicon\.ico|api\/health|admin\/api\/health|admin\/_next\/static)/;
const STATIC_EXT_RE =
...
app
  ...
  .then(() => {
    ...
    server.listen(port, hostname, () => {
      logger.info(
        `game-inventori production server ready on ${hostname}:${port} (${process.env.NODE_ENV ?? "production"})`,
      );
    });
  })
  .catch((err) => {
    logger.error({ err }, "game-inventori production server failed to start");
    process.exit(1);
  });
